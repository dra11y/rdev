...
/// The minimum scroll distance.
const MIN_SCROLL_DISTANCE: f64 = 1f64;
...
pub(crate) struct TouchEventLoop {
    report: SynReport,
    state: TouchpadState,
}
...
fn get_direction(pos1: &Position, pos2: &Position) -> Direction {
    ...
    if (pos2.x - pos1.x).abs() > ((1.5f64 * (pos2.y - pos1.y) as f64) as i32).abs() {
...
struct TouchpadState {
    ...
    pub last_xy: Option<Position>,
    pub scroll_directions: Vec<Direction>,
    pub last_ts: f64,
...
impl SlotState {
    ...
    pub fn push_position(&mut self, x: i32, y: i32) {
        if self.start_xy.is_none() {
            ...
            self.last_xy = Some(pos(x, y));
        } else {
            self.last_xy = self.end_xy.take();
...
impl TouchpadState {
    ...
    fn is_opposite_direction(&self, dir1: Direction, dir2: Direction) -> bool {
        matches!(
            (dir1, dir2),
    ...
    fn update(&mut self, report: &mut SynReport) -> Option<Gesture> {
        ...
        if let (Some(x), Some(y)) = (overall_x.take(), overall_y.take()) {
            ...
            if self.max_fingers == self.last_finger {
                ...
                if Some(Fingers::Two) == self.last_finger {
                    if let (Some(last_xy), Some(end_xy)) = (&self.last_xy, &self.end_xy) {
                        let distance = get_distance(last_xy, end_xy);
                        if distance >= MIN_SCROLL_DISTANCE {
                            let direction = get_direction(last_xy, end_xy);

                            if let Some(&last_direction) = self.scroll_directions.last() {
                                if self.is_opposite_direction(last_direction, direction) {
                                    // Opposite direction detected, clear the list to reset the gesture
                                    self.scroll_directions.clear();
                                }
                            }

                            self.scroll_directions.push(direction);

                            if self.scroll_directions.len() >= 5 {
                                let mut counts: HashMap<&Direction, i32> = HashMap::new();

                                // Count the frequency of each Direction
                                for dir in self.scroll_directions.iter() {
                                    *counts.entry(dir).or_insert(0) += 1;
                                }

                                // Find the Direction with the highest count (mode)
                                let direction = counts
                                    .into_iter()
                                ...
                                if let Some(&direction) = direction {
                                    let distance = match direction {
                                        Direction::Up => (last_xy.y - end_xy.y).abs(),
                                        Direction::Down => (last_xy.y - end_xy.y).abs(),
                                        Direction::Left => (last_xy.x - end_xy.x).abs(),
                                        Direction::Right => (last_xy.x - end_xy.x).abs(),
                                    };
